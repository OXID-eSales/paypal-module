...
Only the `.puml` sources in this directory are kept under version control - the rendered images
are generated on demand into `../SVG/`, which is git-ignored. That way a rendered diagram can
never be out of sync with its source, and the repository carries no binary duplicates of it.
...
**Note:** The `.puml` files in this directory are the source of truth. To change a diagram, edit the PUML file and render it again with `make svg` from `docs/`.
