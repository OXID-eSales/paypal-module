...
$MODULE_ROOT/recipe/parts/shared/prepare_shop_package.sh -e"${edition}" -u"false" -b"b-8.0.x" || exit 1
...
$PROJECT_ROOT/source/extensions/paypal/recipe/parts/shared/require_twig_components.sh -e"${edition}" -b"b-8.0.x" || exit 1
$PROJECT_ROOT/source/extensions/paypal/recipe/parts/shared/require_theme_dev.sh -t"apex" -b"b-8.0.x" || exit 1

# Require demodata package
docker compose exec -T \
  php composer config repositories.oxid-esales/oxideshop-demodata-ee \
  --json '{"type":"git", "url":"https://github.com/OXID-eSales/oxideshop_demodata_ee"}'
docker compose exec -T php composer require oxid-esales/oxideshop-demodata-ee:dev-b-8.0.x --no-update

# Install all preconfigured dependencies
docker compose exec -T php composer update --no-interaction
docker compose exec php bin/oe-console oe:database:reset --force
