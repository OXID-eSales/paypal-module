...
class OrderMain extends OrderMain_parent
{
        ...
        if ($countryCode) {
            ...
            if ($trackingCarrierList->count()) {
                foreach ($trackingCarrierList as $trackingCarrier) {
                    $trackingCarrierId = $trackingCarrier->oscpaypal_trackingcarrier__oxkey->value;
                    $provider[$trackingCarrier->getId()] = [
                        'id'       => $trackingCarrier->oscpaypal_trackingcarrier__oxkey->value,
                        'title'    => $trackingCarrier->oscpaypal_trackingcarrier__oxtitle->value,
    ...
    public function getPayPalTrackingCarrierProviderAsJson()
    {
    ...
    public function getPayPalOrderCountryCode(): string
    {
        ...
        $countryId = $order->oxorder__oxdelcountryid->value ?: $order->oxorder__oxbillcountryid->value;
        $country = oxNew(Country::class);
