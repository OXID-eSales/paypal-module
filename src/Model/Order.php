...
class Order extends Order_parent
{
    ...
    /**
     * Bypass for the proxy-controller skip in _markVouchers. Set to true only
     ...
     * PayPal Standard via proxy) whose mail send paths reach `_markVouchers`
     ...
     * this method is also safe — parent::_markVouchers is idempotent against
    ...
    public function refundPayPalCapture(
        ...
        try {
            ...
            // the property is filled in the constructor, but it is nullable by declaration - and the
            // OXID 7 line of this module fills it lazily, where exactly this call once died on null
            $paymentService = $this->paymentService ?: $this->getServiceFromContainer(PaymentService::class);
     ...
     * path (_getOrderBasket + _addOrderArticlesToBasket + calculateBasket) that
    ...
    {
        /** @var Basket $basket */
        $basket = $this->_getOrderBasket(false);
    ...
    public function finalizeOrder(Basket $basket, $user, $recalculatingOrder = false)
    {
        ...
        // Express _executePayment branch above already covers the common case;
