...
- [0007849](https://bugs.oxid-esales.com/view.php?id=7849): Fix `Twig\Error\LoaderError: Template "@osc_paypal/extensions/themes/admin_twig/headitem.html.twig" is not defined` raised in the admin (`module_main` render after `deactivateModule`) when the PayPal module is deactivated in sequence with another module that also extends the global admin `headitem` (e.g. Amazon Pay). Root cause (OXDEV-9934): extending the global admin `headitem.html.twig` via the Twig theme-chain bakes the module template path into the compiled parent Twig class as a compile-time constant; that compiled class survives every cache clear in PHP-FPM worker memory until the worker dies, so deactivating the module while a worker still holds it crashes the next admin render with the `LoaderError`. Fix: the two backend CSS includes (`bootstrap.min.css`, `paypal-admin.min.css`) were moved out of the global headitem chain extension into a module-local partial that is included directly from the module's own admin templates (`oscpaypalconfig`, `oscpaypalorder`, `oscpaypalorder_pp`, `oscpaypalorder_ppplus`) — these are only rendered when the admin opens a PayPal page, never as a global re-render after a module action, so no module path is baked into a global admin template's compiled class. Applied to both template engines: Twig partial `views/twig/admin/admin_headitem_inccss.html.twig` (auto-resolved via the `@osc_paypal` namespace, no metadata entry needed) and Smarty partial `views/smarty/admin/admin_headitem_inccss.tpl` (registered under `templates` in `metadata.php`). The corresponding `admin_headitem_inccss` headitem block/extension was removed (Twig extension file, Smarty extension file and the Smarty `blocks` entry in `metadata.php`). The Smarty headitem block uses the runtime `blocks` mechanism and is not itself affected by OXDEV-9934, but was migrated as well for symmetry. Files touched: `views/twig/admin/admin_headitem_inccss.html.twig` (new), `views/smarty/admin/admin_headitem_inccss.tpl` (new), `views/twig/admin/oscpaypalconfig.html.twig`, `views/twig/admin/oscpaypalorder.html.twig`, `views/twig/admin/oscpaypalorder_pp.html.twig`, `views/twig/admin/oscpaypalorder_ppplus.html.twig`, `views/smarty/admin/oscpaypalconfig.tpl`, `views/smarty/admin/oscpaypalorder.tpl`, `views/smarty/admin/oscpaypalorder_pp.tpl`, `views/smarty/admin/oscpaypalorder_ppplus.tpl`, `metadata.php`; removed: `views/twig/extensions/themes/admin_twig/headitem.html.twig`, `views/smarty/extensions/themes/admin/admin_headitem_inccss.tpl`.
