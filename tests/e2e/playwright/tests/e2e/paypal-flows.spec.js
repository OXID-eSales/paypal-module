...
async function handlePayPalPopup(popupPage, mainPage) {
    ...
    try {
        ...
        await emailField.waitFor({ state: 'visible', timeout: 15000 }); // Ensure visible
        await emailField.focus(); // Ensure it's active
        await emailField.fill(''); // Clear the field explicitly
        await emailField.fill(email); // Now fill it
